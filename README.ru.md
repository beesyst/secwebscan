...
# Nmap Scan Profiles (SecWebScan)

Этот документ описывает предустановленные профили уровней сканирования для модуля `nmap` в рамках проекта **SecWebScan**. Каждый профиль оптимизирован под определённые задачи, от быстрых проверок до полноценных пентестов.

---

| Уровень   | Описание                                       | Флаги                                                                                     | Особенности                                                                                                     | Применение                                                   |
|-----------|------------------------------------------------|-------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| ✨ easy    | Быстрое и безопасное сканирование              | `-T4 -F -sS -Pn`                                                                          | Быстрое сканирование популярных портов, TCP Half-Open, без пинга                                                | Первая разведка, быстрая проверка                           |
| 🔍 middle  | Подробное сканирование с версией сервисов      | `-T4 -sS -sV -Pn --open`                                                                  | Определение версий, показ только открытых портов                                                                | Детальный обзор после easy                                   |
| ⚖️ hard    | Глубокий анализ цели и базовые скрипты         | `-T4 -sS -sV -O -Pn --script dns-brute,firewalk,http-methods,whois-domain`                | Определение ОС, скрипты на DNS, firewall, HTTP                                                                 | Анализ инфраструктуры и уязвимостей                          |
| ⚡️ extreme | Агрессивный режим со всеми портами и скриптами | `-T4 -p- -sS -sV -O -A -Pn --script all`                                                  | Полный порт-скан, агрессивный режим, все NSE-скрипты                                                           | Полный аудит, CTF, тест на выносливость цели                |
| 🧪 stealth | Скрытный режим обхода обнаружения              | `-T2 -sS -Pn --max-retries 1 --host-timeout 30s`                                          | Замедленный темп, минимум повторов, ограничение времени                                                         | Пентест в среде с IDS/IPS                                     |

---

## ⚙️ Примечания
- Все режимы используют TCP Half-Open (`-sS`) для минимизации обнаружения и лучшей совместимости.
- Флаг `-Pn` (пропуск ICMP ping) используется во всех режимах, чтобы не зависеть от ping-блокировки.

> ⚡ *Эти профили можно легко переключать через поле `"level"` в `config.json` для модуля nmap.*


💡 Общая архитектура пайплайна:
🔧 1. plugin_runner.py
Запускает сканирование (nmap, amass, и т.п.).

Результаты сохраняются в файл (/results/nmap.xml и т.п.).

✅ Ты уже убедился, что nmap.xml создаётся и содержит корректные данные.

📥 2. collector.py
Читает путь module["output"] из config.json, например:

json
Копировать
Редактировать
"output": "results/nmap.xml"
Ищет этот файл внутри контейнера:
...
→ /results/nmap.xml
Загружает плагин (например, plugins/nmap.py), вызывает у него:

python
Копировать
Редактировать
plugin.parse(output_path)
Полученный список результатов вставляет в PostgreSQL, в таблицу nmap_results.

📊 3. report_generator.py
Достаёт данные уже из PostgreSQL:

sql
Копировать
Редактировать
SELECT * FROM nmap_results
Использует шаблоны Jinja (report.html.j2) для генерации терминального и HTML-отчёта.

✅ Ответ на твой вопрос:
Коллектор несет данные из secwebscan/results/nmap.xml в постгресс, правильно?

✔️ Да, и он ищет файл внутри контейнера по пути /results/nmap.xml.

А уже из бд данные парсятся в отчет в терминале?

✔️ Да, терминальный и HTML отчёты берут только из PostgreSQL.
